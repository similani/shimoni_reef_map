...
bbar <- ggplot(urchins.1, aes("", density))+
  ...
  geom_errorbar(aes(ymin=density-0.1, ymax=density+sd), width=.1) +
...
cbar <- ggplot(fishes.1, aes("", biomass))+
  geom_bar(stat = "identity", width = 1, fill = "black")+ #plot a bar chart
  geom_errorbar(aes(ymin = biomass - 30, ymax = biomass+sd), width = .1)+  
...
#main map 
shimoni <- ggplot() +
  geom_sf(data = ke.county, color = 'black', fill = '#DCDCDC')+
  geom_sf(data = kisite, aes(fill = GISNAME), alpha = .25)+
  geom_sf(data = reef, aes(color = ''))+
  geom_sf(data = wasini_cca, fill="#FF4500", alpha = .25)+
  ...
  scale_fill_manual(values = c("#FF4500","#FFA500","black"),
                    name="Legend",
                    labels= c("No-take zone","Marine reserve","Study site"))+
  ...
  theme(panel.grid = element_blank(),
        ...
        legend.title = element_text(size=16, hjust = 0.32, vjust = 1, face = "bold"),
